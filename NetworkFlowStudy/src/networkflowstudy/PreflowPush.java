...
public class PreflowPush {
    ...
    private boolean Push(Vertex v, Vertex w) {
        ...
        if (excess_v > 0) {                                                     // if there is any excess, we can push
            Vertex tempw = graph.getVertex((String)w.getName());                // get the w vertex by name, it may have come from residual graph
            if (heights.get(tempw) < heights.get(v)) {                          // if the height of v is greater than w, we can push
            ...
            while (it.hasNext()) {                                              // check each edge
                ...
                if (edge.getFirstEndpoint().getName().equals(v.getName())) {    // at least as high as v
                    ...
                    if (heights.get(tempw) >= heights.get(v))                   // this is the steepness rule
                        ...
                        if (edge.getSecondEndpoint().getName() != "s")          // except for "s", that's ok
    ...
    private void UpdateExcessMap() {
        ...
        for (int i = 0; i < vertices.size(); i++) {                             // iterate through all vertices
            ...
            if ((ef > 0) && vertices.get(i).getName() != "t")                   // and if this isn't "t"
    ...
    private ArrayList<Vertex> GetWVertexList(Vertex v) {
        boolean hasEdgeVW   = false;                                            // indicate whether an edge was found in the residual graph
        ...
        Iterator graphIterator = graph.incidentEdges(v);                        // iterator of v's incident edges
        Iterator residualIterator = residualGraph.incidentEdges(rV);            // iterator of rV's incident edges
                                                                                //
        while (graphIterator.hasNext()) {                                       // cycle through all incident edges on the graph
            hasEdgeVW = false;                                                  // reset this each time
            Edge edge = (Edge)graphIterator.next();                             // save a copy
            if (edge.getFirstEndpoint().getName().equals(v.getName())) {        // check if v is the first endpoint
                i = residualGraph.edges();                                      // get an iterator to residual graph to locate 'edge'
                while (i.hasNext()) {                                           // iterate over all edges
                    Edge vw_res = (Edge)i.next();                               // save a copy of current edge
                    String e1 = (String)vw_res.getFirstEndpoint().getName();    // save endpoint 1 name
                    String e2 = (String)vw_res.getSecondEndpoint().getName();   // save endpoint 2 name
                    if (e1.equals(v.getName())                                  // if v is the first endpoint,
                            && e2.equals(edge.getSecondEndpoint().getName())) { // and edge.secondEndpoint is the second
                        hasEdgeVW = true;                                       // set this to true
                        break;                                                  // break out of this loop, we're done
                    }                                                           //
                }                                                               //
                if (hasEdgeVW) {                                                // if the edge was in the residual graph
                    Vertex e1 = edge.getFirstEndpoint();                        // save a copy of endpoint 1 and 2
                    Vertex e2 = edge.getSecondEndpoint();                       // if the height of 2 is higher, we'll have to relabel
                    if (heights.get(e2) <= heights.get(e1))                     // so that one should be the last resort. if the heigh
                        w.add(0, edge.getSecondEndpoint());                     // of 2 is the same or lower, we can push right away
                    else                                                        // so that goes first.
                        w.add(edge.getSecondEndpoint());                        //
                }                                                               //
            }                                                                   //
        }                                                                       //
                                                                                //
